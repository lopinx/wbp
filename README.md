...
**关键词文件**（`keywords`）：
- 首行为表头（列名随意），工具自动选择首列为查询词的数据表 sheet（跳过日期/URL 为主的 sheet），取该 sheet首列作为关键词
- 支持多文件，工具随机合并所有文件后选取
- 空行自动过滤
...
| 失效链接 | 0 | 检查前 3 个外链的 HTTP 状态码（GET 请求，4xx 和 5xx 计为失效，网络错误不计） |
...
skills/wpb/scripts/__TEST__/selftest.mjs  # 自检测试套件 (346/346 通过)
