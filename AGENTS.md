...
| `selftest.mjs` | 87 项自动化测试，覆盖语法、TOML 解析（含边界情况）、去重哈希、图片混排、S3 函数、WP API 函数、错误处理、文档验证 |
...
- `node selftest.mjs` 运行完整测试套件（87 项）
- 测试覆盖：语法、init、pick、TOML 解析（含边界情况）、去重（含 CJK）、图片混排（含属性保留）、S3 函数、WP API 函数、helper 函数、错误处理、文档验证、数据文件完整性、错误边界
