...
| `skills/wpb/scripts/__TEST__/selftest.mjs` | 自动化测试 (176/176 通过)，覆盖语法、pick、TOML 解析（含边界情况）、去重哈希、图片混排（含避开小标题/首段前/尾段后、alt/title 派生与 HTML 转义）、图片函数（含 searchImages query 字段功能测试）、WP API 函数、错误处理（含站点字段校验、JSON 解析错误、validateDraft 非对象防护）、文档验证、安装逻辑（doInstall、initConfig、postinstall.mjs 已移除）、CSV/TXT/XLSX/URL 解析、边缘情况（BOM、空行、日期、合并单元格、大数字） |
...
- `node skills/wpb/scripts/__TEST__/selftest.mjs` 运行完整测试套件 (176/176 通过)
